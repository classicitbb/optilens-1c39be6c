...
| 4 | ✅ DONE (2026-07-13, by Claude — both repos were on local disk, so Codex handoff unnecessary) — Doc Studio ported lift-and-shift: public/ds/ copied verbatim from optilens-local (studio.html + support.js + design system + TinyMCE, ~8.9MB); cloud-bridge.js injected (redirects /api/* → docstudio-api edge fn with Supabase session token from same-origin localStorage); docstudio-api edge fn mirrors optilens-local's lib/docstudio.js API exactly (files + billing-documents CRUD/autosave/version-409, shares & users are no-ops, /pl/customers from cloud customers table); tables migration 20260713190000_docstudio_tables.sql; page /admin/website/documents ("Doc Studio" in Website nav). Statement templates (Spec C) live INSIDE the studio as the 'statement' file type — no separate port needed. | **Claude** |
