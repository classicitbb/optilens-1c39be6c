...
// widget sends: sidebar items first, then the app's own base route. Only the
// /admin/ prefix is stripped, so a non-admin route such as /copilot keeps its
// leading slash. The two must agree or a page slug resolves to no context.
