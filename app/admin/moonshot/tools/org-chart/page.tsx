...
import { OrgChartSeat } from "../../lib/types";
      ...
      <Card className="w-[270px]">
        ...
        <CardContent className="space-y-2 pt-0">
          <Select value={seat.assignedUserIds[0] ?? "unassigned"} onValueChange={(value) => onAssign(seat.id, value === "unassigned" ? null : value)}>
            ...
            <SelectContent>
              <SelectItem value="unassigned">Open seat</SelectItem>
              {users.map((user) => (
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button size="sm" variant="outline" onClick={() => onEdit(seat)}>
              Edit
            </Button>
            <Button size="sm" variant="outline" onClick={() => onAddChild(seat.id)}>
              Add child
            </Button>
            <Button size="sm" variant="outline" onClick={() => onToggleCollapse(seat.id)} disabled={children.length === 0}>
              {isCollapsed ? "Expand" : "Collapse"}
            </Button>
            <Button size="sm" variant="destructive" onClick={() => onDelete(seat.id)}>
              Remove
            </Button>
          </div>
        </CardContent>
    ...
    <div className="space-y-4">
      <Card>
        ...
        <CardContent className="flex flex-wrap items-center gap-2">
          <Button variant="outline" onClick={() => setZoom((z) => Math.max(0.6, Number((z - 0.1).toFixed(2))))}>
            Zoom out
          </Button>
          <Button variant="outline" onClick={() => setZoom((z) => Math.min(1.8, Number((z + 0.1).toFixed(2))))}>
            Zoom in
          </Button>
          <Button variant="outline" onClick={() => setOrientation((o) => (o === "vertical" ? "horizontal" : "vertical"))}>
            Orientation: {orientation}
          </Button>
          <Button variant="outline" onClick={() => setExpandedLevel((l) => Math.max(1, l - 1))}>
            Collapse level
          </Button>
          <Button variant="outline" onClick={() => setExpandedLevel((l) => Math.min(10, l + 1))}>
            Expand level
          </Button>
          <Button
            variant="secondary"
      ...
      <Card>
        <CardHeader>
          <CardTitle>Add root seat</CardTitle>
        </CardHeader>
