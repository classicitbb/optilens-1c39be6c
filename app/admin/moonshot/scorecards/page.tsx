          ...
          <Dialog>
            ...
            <DialogContent>
              ...
              <div className="space-y-3">
                <div className="space-y-1"><Label>Who</Label><Select value={ownerId} onValueChange={setOwnerId}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{users.map((u) => <SelectItem key={u.id} value={u.id}>{u.name}</SelectItem>)}</SelectContent></Select></div>
                ...
                <Button className="w-full" onClick={() => {
                  ...
                  addMetric({ name: metricName, ownerId, target: goal, trend: "flat", week: "W1", frequency, unit, actual: points[points.length - 1].value, points });
                }}>Save Metric</Button>
              </div>
            </DialogContent>
          </Dialog>
