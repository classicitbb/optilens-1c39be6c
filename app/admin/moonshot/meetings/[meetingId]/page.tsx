...
export default function MeetingDetailPage() {
  ...
  return (
    <div className="flex gap-4">
      <aside className="w-[320px] shrink-0 rounded-xl border border-slate-700/30 bg-white h-[calc(100vh-120px)] sticky top-20 p-3 flex flex-col">
        ...
        <div className="space-y-1 overflow-auto flex-1">
          {meeting.agenda.map((section) => (
            <button key={section.id} onClick={() => { setActiveSectionId(section.id); setElapsed(0); }} className={`w-full rounded-md px-3 py-2 text-left flex items-center justify-between text-sm ${activeSectionId === section.id ? "bg-teal-50 text-teal-800" : "hover:bg-slate-50"}`}>
