-- Helpdesk staff replies and internal notes rendered as "Unknown" because
-- send_helpdesk_ticket_message only stored sender_user_id and never captured
-- the sender's display name / email, which is what the UI shows.
...
BEGIN
  ...
  SELECT
    NULLIF(btrim(COALESCE(profile.display_name, profile.full_name, '')), ''),
...
-- Backfill messages already stored without a sender identity.
UPDATE public.helpdesk_ticket_messages AS message
SET
  sender_name = COALESCE(message.sender_name, NULLIF(btrim(COALESCE(profile.display_name, profile.full_name, '')), '')),
  sender_email = COALESCE(message.sender_email, NULLIF(btrim(COALESCE(profile.email, '')), ''))
FROM public.profiles AS profile
WHERE profile.user_id = message.sender_user_id
  AND message.sender_user_id IS NOT NULL
  AND (message.sender_name IS NULL OR message.sender_email IS NULL);
