-- Keep a deliberate admin approval effective after the identity RPC resyncs.
-- A linked customer plus a verified email is enough for portal access; profile
-- details can be completed later. Existing CRM contact values are only filled.
...
BEGIN
  ...
  IF v_email_verified AND (v_profile_completed OR v_manual_approved) THEN
    ...
    -- Profile completion can create or enrich a contact. It only fills blank
    -- CRM fields so a portal profile cannot overwrite synced/manual values.
    ...
    -- An explicit admin choice wins. Otherwise resolve an ERP customer from
    -- the contact, its parent company, or an existing account link.
...
GRANT EXECUTE ON FUNCTION public.sync_customer_portal_identity(uuid) TO authenticated, service_role;
