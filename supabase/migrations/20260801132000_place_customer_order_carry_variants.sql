-- Innova integration, step 3: make place_customer_order() carry the variant
-- columns through to order_items.
--
-- DRAFTED FOR REVIEW — not run. You run all migrations yourself.
--
-- ⚠ REVIEW CAREFULLY: this re-creates the live checkout RPC. The body below
-- is copied VERBATIM from 20260723160000_scotia_redirect_mode.sql (the latest
-- definition) with exactly ONE change: the INSERT INTO public.order_items now
-- also writes variant_id / variant_label / variant_sku / variant_opc_code /
-- variant_metadata / unit_price_snapshot, which the client has been sending
-- since the unified-variant-engine migration but the RPC silently dropped.
-- The Rx order form depends on variant_metadata.rx_quote_id surviving into
-- order_items — that's what links a paid order back to its quote so the
-- outbox trigger (20260801131000) can enqueue the Innovations submission.
-- Side benefit: store variant labels/SKUs finally persist on orders too.
