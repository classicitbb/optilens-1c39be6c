...
ALTER TABLE public.order_payments
  ...
  CHECK (status IN ('initiated', 'authorized', 'settled', 'failed', 'refunded', 'void'));
...
BEGIN
  ...
  IF v_is_self_checkout AND v_checkout_method = 'manual_review' THEN
  ...
  IF v_is_self_checkout AND NOT v_should_capture_payment THEN
