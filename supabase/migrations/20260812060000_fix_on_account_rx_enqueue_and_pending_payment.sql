-- Fix two regressions found while testing the Rx order -> Gatekeeper/OptiLens
-- pipeline with 12 on-account test orders (2026-08-12):
--
-- 1. place_customer_order() collapsed every non-capturing checkout method
--    (on_account, stripe_offline, firstpay_offline, bimpay_offline,
--    manual_review) down to a single 'pending' status. That silently broke
--    the offline/manual-review approval path: OrdersPage's "Approve" action
--    and tab only ever look for status = 'pending_payment' (see
--    approve_pending_payment below and src/pages/admin/OrdersPage.tsx), a
--    status this function stopped ever assigning. Those orders now sit
--    forever with no admin action available. Restored the original
--    pending vs pending_payment split: on_account (and the pre-redirect leg
--    of scotia_ecom) still needs no approval step and stays 'pending';
--    stripe_offline/firstpay_offline/bimpay_offline/manual_review genuinely
--    need a human to confirm an offline payment arrived, so they go back to
--    'pending_payment'.
--
-- 2. approve_pending_payment() had lost its own status guard, so it would
--    silently "approve" a payment on any order id it was given (including
--    ones with no payment awaiting approval). Restored the guard.
--
-- 3. The Rx-submission enqueue triggers only fire when an order reaches
--    status = 'confirmed'. On-account orders never do — by design they stay
--    'pending' and go straight to 'shipped' (STATUS_NEXT in
--    OrderDetailDialog.tsx treats 'pending' the same as 'confirmed' for that
--    button). So an on-account Rx order's lines never made it into
--    rx_order_submissions: the checkout page promises "processed
--    immediately," but nothing ever enqueued it for lab release. Broadened
--    the item-level trigger to also enqueue when the order is on_account and
--    still 'pending' — the same condition under which it was actually
--    created and paid for (a credit account, verified at checkout).

-- ── 1. place_customer_order: restore pending vs pending_payment ───────────
...
BEGIN
  ...
  --  - captures payment now                              -> confirmed
  --  - genuinely needs a human to confirm an offline
  --    payment arrived before this can proceed            -> pending_payment
  --  - on_account (credit already verified at checkout)
  --    and the pre-redirect leg of scotia_ecom             -> pending
  v_order_status := CASE
    WHEN v_should_capture_payment THEN 'confirmed'
    WHEN v_checkout_method IN ('stripe_offline', 'firstpay_offline', 'bimpay_offline', 'manual_review') THEN 'pending_payment'
    ELSE 'pending'
  END;
  ...
  -- ⇩ THE ONLY CHANGE vs 20260723160000: variant columns are carried through.
  INSERT INTO public.order_items (
    order_id, product_id, product_name, product_price, product_type, quantity,
    variant_id, variant_label, variant_sku, variant_opc_code, variant_metadata, unit_price_snapshot
  )
  SELECT v_order_id, x.product_id, x.product_name, x.product_price,
    ...
    x.variant_id, x.variant_label, x.variant_sku, x.variant_opc_code,
    COALESCE(x.variant_metadata, '{}'::jsonb), x.product_price
  FROM jsonb_to_recordset(p_items) AS x(
    product_id integer, product_name text, product_price numeric, product_type text, quantity integer,
    variant_id uuid, variant_label text, variant_sku text, variant_opc_code text, variant_metadata jsonb
  );
...
-- ── 2. approve_pending_payment: restore the status guard ───────────────────
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.orders WHERE id = p_order_id AND status = 'pending_payment') THEN
    RAISE EXCEPTION 'Order % is not awaiting payment approval.', p_order_id;
  END IF;
...
-- ── 3. Rx-submission enqueue: on-account orders never reach 'confirmed' ────
-- They're paid (a verified credit line) and dispatched immediately at
-- placement — the checkout page tells the buyer exactly that — so they
-- should enqueue at that point, not wait for a status they will never reach.
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE id = NEW.order_id
      AND (status = 'confirmed' OR (status = 'pending' AND checkout_method = 'on_account'))
  ) THEN
    RETURN NEW;
  END IF;
