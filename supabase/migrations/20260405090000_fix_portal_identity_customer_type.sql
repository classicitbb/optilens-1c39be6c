-- Fix: sync_customer_portal_identity was inserting 'Company'/'Person' into
-- customers.type, which violates the customers_type_check constraint.
-- Remove the type column from the INSERT so it defaults to NULL (allowed).
