...
DROP FUNCTION IF EXISTS public.sync_customer_portal_identity(uuid);
...
BEGIN
  ...
  IF COALESCE(NULLIF(BTRIM(v_profile.organization_name), ''), NULL) IS NOT NULL THEN
    ...
    ORDER BY is_customer DESC, updated_at DESC LIMIT 1;
  END IF;
...
SELECT
  ...
  NULLIF(c.address, ''),
  'Customer',
  COALESCE(NULLIF(c.pipeline_stage, ''), 'Prospect'),
  c.id
FROM public.contacts c
WHERE c.id IN (
  '79ea56a3-909e-4c8f-9568-0f407cfb2642',
  'eb3e0d1c-b261-4dbb-8583-132fbee2b69b',
  'be693bbe-89e6-446a-89cc-fb728b0b4059',
  '9345bbd3-38a2-4187-9a34-26cf2d3be3ca',
  '4faa1abe-4059-45d0-9ca8-fe4403ca1264',
  '8a2d73f0-a381-4a13-940e-f947a9d3046b',
  'cf876296-9496-4516-8bb7-bacb1e36b992',
  'b94c4a1e-887d-4502-8166-66ce57908abc',
  '4c37fb7f-52e5-4885-91be-d4542877e7e0'
)
AND NOT EXISTS (SELECT 1 FROM public.customers cu WHERE cu.contact_id = c.id);
