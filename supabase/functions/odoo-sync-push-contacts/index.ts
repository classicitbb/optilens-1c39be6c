...
import { corsHeaders, createRunLog, finalizeRunLog, loadConnection, supabaseAdmin } from "../_shared/odoo/runtime.ts";
...
serve(async (req) => {
  ...
  const run = await createRunLog(connection.id, "push");

  try {
    const result = await pushContacts(connection.id);
    await finalizeRunLog(run.id, {
      status: "success",
      pushRecords: result.processed,
    ...
    await supabaseAdmin.from("integration_connections").update({
      last_sync_export_count: result.processed,
