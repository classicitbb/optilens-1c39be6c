...
var ADMIN_RESOURCE_BY_KEY = new Map(ADMIN_RESOURCES.map((resource) => [resource.key, resource]));
var findAdminResource = (key) => {
...
var classifyAdminWrite = (resource, operation) => {
  if (operation === "delete") {
...
var sanitizeLike = (value) => `%${value.replaceAll(",", " ").replaceAll("(", " ").replaceAll(")", " ").replaceAll("%", "").replaceAll("_", "").trim()}%`;
var pickWritable = (resource, values) => {
...
var RESOURCE_KEYS_HINT = ADMIN_RESOURCES.map((resource) => resource.key).join(", ");
var ADMIN_RESOURCE_TOOLS = [
  {
...
var ADMIN_RESOURCE_TOOL_NAMES = new Set(ADMIN_RESOURCE_TOOLS.map((tool) => tool.name));

// src/lib/mcp/tools/admin-list-resources.ts
var admin_list_resources_default = defineTool24({
  name: "admin_list_resources",
...
var admin_search_records_default = defineTool25({
  name: "admin_search_records",
...
var admin_get_record_default = defineTool26({
  name: "admin_get_record",
...
var admin_write_record_default = defineTool27({
