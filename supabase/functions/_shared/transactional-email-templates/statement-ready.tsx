...
const StatementReadyEmail = ({ customerName = 'there', accountNumber = '', periodStart, periodEnd, closingBalance = 0, dueDate, siteUrl = 'https://classicvisions.net', unsubscribeUrl }: StatementReadyProps) => (
  <ClassicVisionsEmailLayout preview={`Your Classic Visions statement is ready — balance $${closingBalance.toFixed(2)}`} title="Your statement is ready" unsubscribeUrl={unsubscribeUrl}>
...
const summarySection = { backgroundColor: '#F4F2ED', borderRadius: '8px', padding: '16px 20px', margin: '0 0 20px' }
