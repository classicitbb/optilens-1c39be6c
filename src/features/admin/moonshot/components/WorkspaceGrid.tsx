...
import type { WorkspaceTileType } from "@/features/admin/moonshot/lib/types";
...
export function WorkspaceGrid({ scope }: { scope: Scope }) {
  ...
  const {
    metrics, rocks, todos, issues, coreValues, privateNotes, headlines, quickLinks,
    dashboardTiles, workspaceTiles,
    addTile, removeTile, moveTile, resizeTile, updateTodo, updatePrivateNotes,
  } = useMoonshotStore();
      ...
      case "quick-links":
        return (
          <div className="grid gap-2 sm:grid-cols-2">
            {quickLinks.map((link) => (
              <Button key={link.href} variant="outline" className="justify-between" onClick={() => navigate(link.href)}>
                {link.label}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {tiles.map((tile, idx) => (
          <Card
            ...
            className={`rounded-xl border bg-card ${tile.colSpan >= 4 ? "xl:col-span-4" : tile.colSpan === 3 ? "xl:col-span-3" : tile.colSpan === 2 ? "xl:col-span-2" : "xl:col-span-1"}`}
          >
