...
import type { MetricFrequency } from "@/features/admin/moonshot/lib/types";
  ...
  return (
    <div className="inline-flex rounded-full border bg-muted p-1">
      {frequencyOrder.map((f) => (
        <button key={f} onClick={() => onChange(f)} className={cn("px-3 py-1 text-sm rounded-full capitalize", value === f ? "bg-background shadow text-foreground" : "text-muted-foreground")}>
          {f}
        </button>
      ))}
    </div>
