...
describe("voice entry surfaces", () => {
  ...
  it("offers the same microphone picker on the floating widget and the full console, remembered per browser", () => {
    const assistant = read("src/components/admin/copilot/AdminCopilotAssistant.tsx");
    const consolePage = read("src/pages/admin/PortalCopilotPage.tsx");
    ...
    expect(consolePage).toContain('from "@/features/admin/copilot/VoiceSettingsMenu"');
    ...
    // A microphone that is later unplugged must not strand the user.
    expect(hook).toContain("overconstrained");
  });
