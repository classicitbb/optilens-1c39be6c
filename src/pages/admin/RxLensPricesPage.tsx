...
import { usePricelistVersions, PricelistVersion, CreateVersionInput } from "@/hooks/usePricelistVersions";
...
import {
  ChevronDown, ChevronRight, Plus, Pencil, Trash2, Copy, Loader2, CalendarDays, Clock } from
"lucide-react";
...
const RxLensPricesPage = () => {
  ...
  const resetForm = () => {
    setName("");setCopyFrom("matrix");setCurrency("BBD");
    setMarkupPct("0");setDiscountPct("0");setIsTemplate(false);setEditMode(null);
  };
  ...
  const handleSave = () => {
    ...
    if (editMode) {
      updateMutation.mutate(
        { id: editMode.id, updates: { name: name.trim(), base_currency: currency, markup_percent: parseFloat(markupPct) || 0, discount_percent: parseFloat(discountPct) || 0, is_template: isTemplate } },
        { onSuccess: () => {setDialogOpen(false);resetForm();toast({ title: "Pricelist updated" });}, onError: (e: any) => toast({ title: "Error", description: e.message, variant: "destructive" }) }
      );
    } else {
      const input: CreateVersionInput = {
        name: name.trim(), base_currency: currency, markup_percent: parseFloat(markupPct) || 0,
        discount_percent: parseFloat(discountPct) || 0, is_template: isTemplate,
  ...
  const handleDelete = (v: PricelistVersion) => {
    ...
    deleteMutation.mutate(v.id, {
      onSuccess: () => {if (resolvedVersionId === v.id) setSelectedVersionId(null);toast({ title: "Deleted" });},
      onError: (e: any) => toast({ title: "Error", description: e.message, variant: "destructive" })
    });
  };
  ...
  const handleDuplicate = (v: PricelistVersion) => {
    const input: CreateVersionInput = {
      name: `${v.name} (Copy)`, base_currency: v.base_currency ?? "BBD",
      markup_percent: v.markup_percent ?? 0, discount_percent: v.discount_percent ?? 0,
      is_template: false, copyFrom: v.id
    };
    createMutation.mutate(input, {
      onSuccess: (newV: any) => {setSelectedVersionId(newV.id);toast({ title: "Duplicated" });},
      onError: (e: any) => toast({ title: "Error", description: e.message, variant: "destructive" })
    });
  };
  ...
  return (
    <div className="p-4 space-y-4 border">
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    border: isActive ? `1px solid hsl(215 65% 50% / 0.3)` : "1px solid transparent"
                  }}
                  onClick={() => setSelectedVersionId(v.id)}>

                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-semibold truncate" style={{ color: isActive ? BLUE : "hsl(215 30% 15%)" }}>
                          {v.name}
                        </span>
                        {v.is_template &&
                      <Badge variant="outline" className="text-[9px] px-1 py-0 h-3.5" style={{ borderColor: BLUE, color: BLUE }}>
                            Template
                          </Badge>
                      }
                      ...
                      <div className="flex items-center gap-3 mt-0.5">
                        <span className="text-[10px]" style={{ color: LABEL_COLOR }}>
                          Markup: {v.markup_percent ?? 0}% · Discount: {v.discount_percent ?? 0}%
                        </span>
                        <span className="text-[10px]" style={{ color: "hsl(215 15% 60%)" }}>
                          Created {v.created_at ? format(new Date(v.created_at), "dd MMM yyyy") : "—"}
                        </span>
                      </div>
                    </div>
                    {canEdit &&
                  <div className="flex items-center gap-0.5 shrink-0" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => openEdit(v)} className="p-1 rounded hover:bg-black/5" title="Edit">
                          <Pencil className="h-3 w-3" style={{ color: LABEL_COLOR }} />
                        </button>
                        <button onClick={() => handleDuplicate(v)} className="p-1 rounded hover:bg-black/5" title="Duplicate" disabled={createMutation.isPending}>
                          <Copy className="h-3 w-3" style={{ color: LABEL_COLOR }} />
                        </button>
                        {isAdmin &&
                    <button onClick={() => handleDelete(v)} className="p-1 rounded hover:bg-red-50" title="Delete" disabled={deleteMutation.isPending}>
      ...
      <div
        ...
        style={{ background: "hsl(215 65% 50% / 0.05)", borderColor: "hsl(215 65% 50% / 0.2)" }}>
          ...
          <div className="flex-1 space-y-0.5">
            ...
            <div className="flex items-center gap-4 text-[10px]" style={{ color: LABEL_COLOR }}>
              ...
              {activeVersion.updated_at && activeVersion.updated_at !== activeVersion.created_at &&
            <span className="flex items-center gap-1">
                  <Clock className="h-3 w-3" />
                  Last saved{" "}
                  {format(new Date(activeVersion.updated_at), "dd MMM yyyy 'at' HH:mm")}
                </span>
            }
            </div>
          </div>
      ...
      <div className="space-y-4">
          ...
          <button
            ...
            style={{ color: contentTab === tab ? "hsl(215 30% 15%)" : LABEL_COLOR }}>

                {tab === "matrix" ? "Price Matrix" : "List Catalog"}
                {contentTab === tab &&
            <span className="absolute bottom-0 left-0 right-0 h-0.5" style={{ background: BLUE }} />
            }
              </button>
          )}
          </div>
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-4 py-2">
            <div>
              ...
              <Input value={name} onChange={(e) => setName(e.target.value)} className="h-8 text-xs" placeholder="e.g. Retail RX Feb 2026" autoFocus />
            </div>
            {!editMode &&
            <div>
                <label className="text-xs font-medium mb-1 block" style={{ color: LABEL_COLOR }}>Copy Prices From</label>
                <Select value={copyFrom} onValueChange={setCopyFrom}>
                  <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="matrix" className="text-xs">📊 From Price Matrix (baseline)</SelectItem>
                    {versions?.map((v) =>
            ...
            <div>
              <label className="text-xs font-medium mb-1 block" style={{ color: LABEL_COLOR }}>Base Currency</label>
              <div className="flex gap-2">
                {(["BBD", "USD"] as const).map((c) =>
                <button key={c} onClick={() => setCurrency(c)} className="flex-1 py-1.5 text-xs font-medium rounded border transition-colors" style={{ background: currency === c ? BLUE : "transparent", color: currency === c ? "white" : LABEL_COLOR, borderColor: currency === c ? BLUE : "hsl(215 15% 80%)" }}>{c}</button>
                )}
              </div>
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => {setDialogOpen(false);resetForm();}}>Cancel</Button>
            <Button size="sm" className="h-7 text-xs" style={{ background: BLUE, color: "white" }} onClick={handleSave} disabled={!name.trim() || createMutation.isPending || updateMutation.isPending}>
