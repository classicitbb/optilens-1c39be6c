      ...
      <CardContent className="space-y-2">
        ...
        {rocks.map((r) => (
          <div key={r.id} className="border rounded p-3 flex justify-between">
            <button onClick={() => updateRock(r.id, { status: r.status === "On Track" ? "At Risk" : "On Track" })}>{r.title} · {r.status}</button>
            <Button variant="destructive" size="sm" onClick={() => deleteRock(r.id)}>Delete</Button>
          </div>
        ))}
      </CardContent>
    </Card>
