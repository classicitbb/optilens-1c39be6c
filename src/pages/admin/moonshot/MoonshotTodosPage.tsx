        ...
        {todos.map((t) => (
          <div key={t.id} className="border rounded p-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Checkbox checked={t.completed} onCheckedChange={(checked) => updateTodo(t.id, { completed: Boolean(checked) })} />
              {t.title}
            </div>
            <Button variant="destructive" size="sm" onClick={() => deleteTodo(t.id)}>Delete</Button>
          </div>
        ))}
      </CardContent>
    </Card>
