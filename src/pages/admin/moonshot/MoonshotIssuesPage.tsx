...
export default function MoonshotIssuesPage() {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-2">
        ...
        {issues.map((i) => (
          <div key={i.id} className="border rounded p-3 flex justify-between">
            <button onClick={() => updateIssue(i.id, { status: i.status === "Open" ? "Resolved" : "Open" })}>{i.title} · {i.status}</button>
            <Button variant="destructive" size="sm" onClick={() => deleteIssue(i.id)}>Delete</Button>
          </div>
        ))}
      </CardContent>
    </Card>
