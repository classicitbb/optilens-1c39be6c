...
import type { OrgChartSeat } from "@/features/admin/moonshot/lib/types";
...
function SeatNode({
  ...
  onToggleCollapse, onAssign, onAddChild, onEdit, onDelete,
}: SeatNodeProps) {
        ...
        <CardHeader className="space-y-2 pb-3">
          ...
          <div className="flex flex-wrap gap-1">
            {assignedUsers.length > 0 ? (
              ...
              <Badge variant="destructive" className="text-xs">Open seat</Badge>
            )}
          </div>
        </CardHeader>
      ...
      {children.length > 0 && !isCollapsed && (
        <div className={orientation === "vertical" ? "mt-4 flex flex-col items-center" : "ml-4 flex items-center"}>
          ...
          <div className={orientation === "vertical" ? "flex flex-wrap justify-center gap-5" : "flex flex-col gap-5"}>
            {children.map((child) => (
              <SeatNode
                key={child.id} seat={child} byId={byId} users={users}
                collapsed={collapsed} expandedLevel={expandedLevel} depth={depth + 1}
                orientation={orientation} onToggleCollapse={onToggleCollapse}
                onAssign={onAssign} onAddChild={onAddChild} onEdit={onEdit} onDelete={onDelete}
              />
            ))}
          </div>
        </div>
      ...
      <div className="overflow-auto rounded-lg border bg-card p-6">
        <div style={{ transform: `scale(${zoom})`, transformOrigin: "top left" }}>
          <div className={orientation === "vertical" ? "space-y-8" : "space-y-6"}>
            {roots.map((root) => (
              <SeatNode
                key={root.id} seat={root} byId={byId} users={users}
                collapsed={collapsed} expandedLevel={expandedLevel} depth={0}
                orientation={orientation} onToggleCollapse={onToggleCollapse}
