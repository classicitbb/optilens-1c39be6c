...
export default function MoonshotMeetingsPage() {
    ...
    <div className="space-y-4">
          ...
          <DialogContent>
            ...
            <div className="space-y-3">
              ...
              <Button className="w-full" onClick={() => {
                addMeeting({ title, owner: "Classic", date: format(new Date(), "yyyy-MM-dd"), status: "Scheduled", notes: "", frequency, duration, attendeeIds: attendees });
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {meetings.map((meeting) => {
          ...
          return (
            <Card key={meeting.id} className="rounded-xl border bg-card">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex -space-x-2">
                    {attendeeList.slice(0, 4).map((u) => (
                      <Avatar key={u.id} className="h-8 w-8 border-2 border-background"><AvatarFallback>{u.avatar}</AvatarFallback></Avatar>
                    ))}
                  </div>
