...
const ShipmentDetailPage = () => {
  ...
  const { data: charges = [], upsertMutation: upsertCharge, deleteMutation: deleteCharge } = useShipmentCharges(id ?? null);
  const { data: lines = [], upsertMutation: upsertLine, deleteMutation: deleteLine } = useShipmentLines(id ?? null);
          ...
          <div className="border rounded overflow-auto">
            <Table>
              ...
              <TableBody>
                {lines.map((l) => {
                  ...
                  return (
                    <TableRow key={l.id} className="text-xs">
                      <TableCell className="py-1">
                        <Select value={l.product_type} disabled={!editable} onValueChange={(v) => upsertLine.mutate({ ...l, product_type: v as any })}>
                      ...
                      <TableCell className="py-1">
                        <Input className="h-7 text-xs w-40" value={l.description} disabled={!editable}
                ...
                {lines.length === 0 && (
                  <TableRow><TableCell colSpan={12} className="text-center text-xs py-4 text-muted-foreground">No line items</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </div>
