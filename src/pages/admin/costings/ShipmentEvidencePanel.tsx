...
import { FileText, ImageIcon, Link2, Upload, Unlink } from "lucide-react";
...
const ShipmentEvidencePanel = ({ shipmentId, targets, readOnly = false }: { shipmentId: string | null; targets: EvidenceTarget[]; readOnly?: boolean }) => {
  ...
  return <section className="flex min-h-[410px] flex-1 flex-col rounded-lg border border-border bg-card shadow-sm">
    <div className="flex items-center justify-between border-b px-3 py-2"><div><h2 className="text-xs font-semibold">Document review</h2><p className="text-[10px] text-muted-foreground">Manual evidence links only; documents never overwrite costing values.</p></div><Badge variant="outline" className="text-[9px]">PDF reader</Badge></div>
