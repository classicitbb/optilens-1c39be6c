...
const PricingComparePage = () => {
  ...
  const visibleLenses = useMemo(() => {
    const deduped = new Map<string, Lens>();
    for (const lens of lenses) {
      if (!lens.is_active || (!lens.show_in_pricelist && lens.sell_price <= 0)) continue;
      if (!deduped.has(lens.id)) deduped.set(lens.id, lens);
    }
  ...
  const filteredByColumn = useMemo(() => {
    ...
    const filterFor = (column: CompareColumn) => {
      ...
      const selectedForColumn = selectedLensIdsByColumn[column];
      const blockedLensIds = new Set(
        COLUMNS
          .map((entry) => selectedLensIdsByColumn[entry])
          .filter((entry): entry is string => Boolean(entry))
          .filter((entry) => entry !== selectedForColumn)
      );
  ...
  const selectedLensIds = useMemo(
    () => new Set(selected.map((slot) => slot.lens?.id).filter((entry): entry is string => Boolean(entry))),
    [selected]
  );
  ...
  const addLensToCompare = (column: CompareColumn, lens: Lens) => {
    setSelected((prev) => {
      const existsInOtherColumn = prev.some((slot) => slot.column !== column && slot.lens?.id === lens.id);
      if (existsInOtherColumn) return prev;
      return prev.map((slot) => (slot.column === column ? { ...slot, lens } : slot));
    });
  };
                ...
                <TableBody>
                  {isLoading ? (
                    ...
                    filteredByColumn[column].map((lens) => {
                      ...
                      const selectedIdInColumn = selectedByColumn.get(column)?.id ?? null;
                      const isSelectedElsewhere = selectedLensIds.has(lens.id) && selectedIdInColumn !== lens.id;
                      const supplierColor =
                        preference === "disliked"
                          ...
                          <TableCell className="align-top">
                            <div className="flex items-center justify-end gap-0.5">
                              <Button
                                ...
                                size="icon"
                                className="h-6 w-6"
                                onClick={() => addLensToCompare(column, lens)}
                                title={isSelectedElsewhere ? "Already selected in another compare column" : "Add to compare"}
