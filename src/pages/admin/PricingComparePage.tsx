...
const PricingComparePage = ({ onClose }: Props) => {
  ...
  const filteredByColumn = useMemo(() => {
    ...
    const filterFor = (column: CompareColumn) => {
      ...
      if (!query) return columnBase.slice(0, 50);

      // Exact substring matching for predictable results
      return columnBase
        .filter((lens) => {
          const fields = [
            lens.name,
  ...
  const setSearchValue = (column: CompareColumn, value: string) => {
    setSearchByColumn((prev) => {
      if (linkedSearch && column === 1) return { 1: value, 2: value, 3: value };
  ...
  const toggleLinkedSearch = () => {
    setLinkedSearch((prev) => {
      const next = !prev;
      if (next) setSearchByColumn((current) => ({ 1: current[1], 2: current[1], 3: current[1] }));
      return next;
    });
  };
  ...
  if (isMobile && !onClose) return <div className="h-full" />;
    ...
    <div className="h-full flex flex-col gap-4 p-4 overflow-hidden">
      <div className="flex items-center justify-between shrink-0 gap-2">
        ...
        <div className="flex items-center gap-1.5 flex-wrap justify-end">
          ...
          {onClose && (
            <Button variant="ghost" size="icon" className="h-7 w-7 ml-2" onClick={onClose} title="Close compare tool">
              <X className="h-4 w-4" />
            </Button>
          )}
        </div>
      ...
      <div className="grid grid-cols-4 gap-3 min-h-0 flex-1">
          ...
          <div key={column} className="border rounded-md flex flex-col min-h-0" style={{ borderColor: "hsl(var(--admin-border))" }}>
            <div className="p-2 border-b" style={{ borderColor: "hsl(var(--admin-border))" }}>
              <div className="mb-2 flex items-center justify-between gap-1">
                <p className="text-xs font-semibold">Column {column}</p>
                {column === 1 && (
                  <Button
                    type="button"
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  placeholder="Type to search exactly…"
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-[11px] text-right">USD</TableHead>
                    <TableHead className="text-[11px] text-right">BBD</TableHead>
                ...
                <TableBody>
                  {isLoading ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={5} className="text-xs text-center py-4 text-muted-foreground">
                        {searchByColumn[column] ? "No exact matches." : "No lenses."}
                      </TableCell>
                    </TableRow>
                  ) : (
                    filteredByColumn[column].map((lens) => {
                      ...
                      const nameColor =
                        preference === "disliked" ? "hsl(var(--admin-destructive))"
                        : preference === "liked" ? "hsl(var(--admin-success))"
                        : "hsl(var(--admin-content-fg))";
                      return (
                        <TableRow key={`${column}-${lens.id}`}>
                          <TableCell className="text-[11px] align-top py-1">
                            <p className="font-medium leading-tight" style={{ color: nameColor }}>{lens.name}</p>
                          ...
                          <TableCell className="align-top py-1">
                            <div className="flex items-center justify-end gap-0.5">
                              ...
                              <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => setPreference(lens.id, preference === "liked" ? null : "liked")} title="Like">
                                <ThumbsUp className="h-3 w-3" style={{ color: preference === "liked" ? "hsl(var(--admin-success))" : undefined }} />
                              </Button>
                              <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => setPreference(lens.id, preference === "disliked" ? null : "disliked")} title="Dislike">
        ...
        <div className="border rounded-md flex flex-col min-h-0" style={{ borderColor: "hsl(var(--admin-border))" }}>
          <div className="p-2 border-b" style={{ borderColor: "hsl(var(--admin-border))" }}>
            ...
            <p className="text-[11px] text-muted-foreground">Selected lenses vs column 1.</p>
          </div>
          <div className="min-h-0 overflow-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="text-[11px]">Col</TableHead>
                  <TableHead className="text-[11px]">Lens</TableHead>
              ...
              <TableBody>
                {selected.map((slot) => {
                  ...
                  if (slot.column !== 1 && baseline != null) {
                    deltaLabel = diffMode === "absolute"
                      ? (metric === "markup_percent" ? `${(deltaAbs ?? 0).toFixed(2)} pp` : currency(deltaAbs ?? 0))
                      : `${(deltaPct ?? 0).toFixed(2)}%`;
                  }

                  const deltaColor = slot.column !== 1 && deltaAbs != null
                    ? (deltaAbs < 0 ? "hsl(var(--admin-success))" : deltaAbs > 0 ? "hsl(var(--admin-destructive))" : undefined)
                    : undefined;
                  ...
                  return (
                    <TableRow key={`selected-${slot.column}`}>
                      ...
                      <TableCell className="text-xs text-right" style={{ color: deltaColor }}>
                        {slot.column === 1 ? "Baseline" : deltaLabel}
                      </TableCell>
                      <TableCell className="text-right">
                        <Button variant="ghost" size="icon" className="h-6 w-6"
                          onClick={() => setSelected((prev) => prev.map((entry) => entry.column === slot.column ? { ...entry, lens: null } : entry))}
                          title="Remove"
                        >
                          <X className="h-3 w-3" />
                        </Button>
