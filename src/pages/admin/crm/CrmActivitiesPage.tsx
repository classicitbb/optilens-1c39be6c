...
import {
  ...
  PhoneCall, PlusCircle, Search, Sparkles, UserRound, UsersRound,
} from "lucide-react";
...
const CrmActivitiesPage = () => {
  ...
  const openDialog = (key: "createActivity" | "editActivity" | "logCall", value: string) => { const params = new URLSearchParams(location.search); params.delete("createActivity"); params.delete("editActivity"); params.delete("logCall"); params.set(key, value); navigate({ pathname: location.pathname, search: `?${params}` }); };
  ...
  return <div className="space-y-4"><AdminPageHeader title="CRM Focus Desk" icon={Columns3}><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => openDialog("logCall", "1")}><PhoneCall className="mr-1 h-4 w-4" />Log call</Button><Button size="sm" onClick={() => openDialog("createActivity", "1")}><PlusCircle className="mr-1 h-4 w-4" />New task</Button></div></AdminPageHeader>
