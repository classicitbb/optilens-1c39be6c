...
type QboState = { environment: "sandbox" | "production"; status: string; company_name: string | null; realm_id_masked: string | null; connected_at: string | null; last_refresh_at: string | null; last_reconciliation_status: string | null; last_reconciliation_at: string | null; last_error_message_sanitized: string | null };
const date = (value: string | null) => value ? new Date(value).toLocaleString() : "—";
<<<<<<< Updated upstream
const qboEnvironment = import.meta.env.VITE_QBO_ENVIRONMENT === "production" ? "production" : "sandbox";
=======
const environment = (import.meta.env.VITE_QBO_ENVIRONMENT || "sandbox") as "sandbox" | "production";
>>>>>>> Stashed changes
...
<<<<<<< Updated upstream
    const { data, error } = await (supabase as any).from("qbo_integration_state").select("status,company_name,realm_id_masked,connected_at,last_refresh_at,last_reconciliation_status,last_reconciliation_at,last_error_message_sanitized").eq("provider", "quickbooks_online").eq("environment", qboEnvironment).maybeSingle();
=======
...
>>>>>>> Stashed changes
  ...
  return <Card>
<<<<<<< Updated upstream
    <CardHeader><CardTitle className="flex items-center justify-between text-base"><span>QuickBooks Online</span><Badge variant="outline">{status.replaceAll("_", " ")}</Badge></CardTitle><CardDescription>{qboEnvironment === "production" ? "Production" : "Sandbox"} · Accounting. Credentials and tokens are managed server-side only.</CardDescription></CardHeader>
=======
