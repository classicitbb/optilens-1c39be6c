...
const AddonDataTable = ({ addons, search, canEdit, onRowClick, onToggleActive, onDuplicate, onDelete, canDelete }: Props) => {
  ...
  const filtered = useMemo(() => {
    let items = addons;
    if (filter === "active") items = items.filter((i) => i.is_active);else
    if (filter === "inactive") items = items.filter((i) => !i.is_active);else
    if (filter === "web") items = items.filter((i) => i.show_on_website);
      ...
      <Table>
        ...
        <TableBody>
          {visible.map((a, idx) => {
              ...
              if (a.cost === 0) rowBg = "hsl(0 70% 96%)";else
              if (a.price > 0 && a.price <= a.cost * fxRate) rowBg = "hsl(0 70% 95%)";else
              if (a.price > 0) {
              ...
              <TableCell className={tdCls}>
                <span
                      className="inline-block text-[10px] font-medium px-1.5 rounded py-px"
                      style={{
