...
interface QuotePdfExportProps {
  ...
  frameData?: {
    ref: string; model: string; bridge: string; ed: string;
        ...
        {frameData && (frameData.ref || frameData.model || frameData.a) && (
          <div className="section">
            ...
            <div className="frame-grid">
              {frameData.ref && <div><div className="field-label">Brand / Ref</div><div className="field-value">{frameData.ref}</div></div>}
              {frameData.model && <div><div className="field-label">Model / Color</div><div className="field-value">{frameData.model}</div></div>}
              {frameData.a && <div><div className="field-label">Eye A (mm)</div><div className="field-value">{frameData.a}</div></div>}
              {frameData.b && <div><div className="field-label">Eye B (mm)</div><div className="field-value">{frameData.b}</div></div>}
              {frameData.bridge && <div><div className="field-label">Bridge (mm)</div><div className="field-value">{frameData.bridge}</div></div>}
              {frameData.ed && <div><div className="field-label">ED (mm)</div><div className="field-value">{frameData.ed}</div></div>}
              {frameData.dbl && <div><div className="field-label">DBL (mm)</div><div className="field-value">{frameData.dbl}</div></div>}
              <div><div className="field-label">Edge</div><div className="field-value">{frameData.uncut ? `Uncut${frameData.uncutPrice ? ` $${frameData.uncutPrice}` : ""}` : "Edged"}</div></div>
            </div>
          </div>
        )}
    ...
    <div className="border border-border rounded-lg overflow-hidden bg-white shadow-sm">
      ...
      <div className="print-root" style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", color: "#1a202c", padding: "32px", fontSize: "11px", maxHeight: "600px", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "24px", borderBottom: "3px solid #2b6cb0", paddingBottom: "16px" }}>
          <div>
            <div style={{ fontSize: "20px", fontWeight: 700, color: "#2b6cb0" }}>OptiLens Pro</div>
            <div style={{ fontSize: "10px", color: "#718096", marginTop: "2px" }}>Precision Optics & Lens Solutions</div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: "16px", fontWeight: 700 }}>{quote.quote_number}</div>
            <div style={{ display: "inline-block", background: "#ebf4ff", color: "#2b6cb0", padding: "2px 8px", borderRadius: "3px", fontSize: "10px", fontWeight: 600, marginTop: "4px" }}>
              {quote.quote_type} QUOTE
            </div>
            <div style={{ fontSize: "10px", color: "#4a5568", marginTop: "3px" }}>Date: {formatDate(quote.created_at)}</div>
            {quote.valid_until && <div style={{ fontSize: "10px", color: "#4a5568" }}>Valid Until: {formatDate(quote.valid_until)}</div>}
          </div>
        </div>

        {/* Customer */}
        <div style={{ marginBottom: "20px" }}>
          <div style={{ fontSize: "10px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.5px", color: "#718096", marginBottom: "8px" }}>Customer Details</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            <div><div style={{ fontSize: "9px", color: "#a0aec0", textTransform: "uppercase" }}>Customer</div><div style={{ fontSize: "11px" }}>{quote.customer_name || "—"}</div></div>
            <div><div style={{ fontSize: "9px", color: "#a0aec0", textTransform: "uppercase" }}>Contact</div><div style={{ fontSize: "11px" }}>{quote.contact_name || "—"}</div></div>
            {quote.contact_email && <div><div style={{ fontSize: "9px", color: "#a0aec0", textTransform: "uppercase" }}>Email</div><div style={{ fontSize: "11px" }}>{quote.contact_email}</div></div>}
            {quote.contact_phone && <div><div style={{ fontSize: "9px", color: "#a0aec0", textTransform: "uppercase" }}>Phone</div><div style={{ fontSize: "11px" }}>{quote.contact_phone}</div></div>}
          </div>
        </div>
        ...
        {frameData && (frameData.ref || frameData.model || frameData.a) && (
          <div style={{ marginBottom: "20px" }}>
            <div style={{ fontSize: "10px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.5px", color: "#718096", marginBottom: "8px" }}>Frame Details</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "10px" }}>
              {frameData.ref && <div><div style={{ fontSize: "9px", color: "#a0aec0", textTransform: "uppercase" }}>Brand/Ref</div><div style={{ fontSize: "11px" }}>{frameData.ref}</div></div>}
              {frameData.model && <div><div style={{ fontSize: "9px", color: "#a0aec0", textTransform: "uppercase" }}>Model</div><div style={{ fontSize: "11px" }}>{frameData.model}</div></div>}
              {frameData.a && <div><div style={{ fontSize: "9px", color: "#a0aec0", textTransform: "uppercase" }}>Eye A</div><div style={{ fontSize: "11px" }}>{frameData.a}mm</div></div>}
              {frameData.b && <div><div style={{ fontSize: "9px", color: "#a0aec0", textTransform: "uppercase" }}>Eye B</div><div style={{ fontSize: "11px" }}>{frameData.b}mm</div></div>}
              {frameData.bridge && <div><div style={{ fontSize: "9px", color: "#a0aec0", textTransform: "uppercase" }}>Bridge</div><div style={{ fontSize: "11px" }}>{frameData.bridge}mm</div></div>}
              {frameData.ed && <div><div style={{ fontSize: "9px", color: "#a0aec0", textTransform: "uppercase" }}>ED</div><div style={{ fontSize: "11px" }}>{frameData.ed}mm</div></div>}
              <div><div style={{ fontSize: "9px", color: "#a0aec0", textTransform: "uppercase" }}>Edge</div><div style={{ fontSize: "11px" }}>{frameData.uncut ? "Uncut" : "Edged"}</div></div>
            </div>
          </div>
        )}
        ...
        {/* Lines */}
        <div style={{ marginBottom: "20px" }}>
          <div style={{ fontSize: "10px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.5px", color: "#718096", marginBottom: "8px" }}>Line Items</div>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "#f7fafc" }}>
                <th style={{ textAlign: "left", padding: "6px 8px", fontSize: "9px", fontWeight: 600, textTransform: "uppercase", color: "#4a5568", borderBottom: "2px solid #e2e8f0" }}>#</th>
                <th style={{ textAlign: "left", padding: "6px 8px", fontSize: "9px", fontWeight: 600, textTransform: "uppercase", color: "#4a5568", borderBottom: "2px solid #e2e8f0" }}>Description</th>
                <th style={{ textAlign: "right", padding: "6px 8px", fontSize: "9px", fontWeight: 600, textTransform: "uppercase", color: "#4a5568", borderBottom: "2px solid #e2e8f0" }}>Qty</th>
                <th style={{ textAlign: "right", padding: "6px 8px", fontSize: "9px", fontWeight: 600, textTransform: "uppercase", color: "#4a5568", borderBottom: "2px solid #e2e8f0" }}>Unit Price</th>
                <th style={{ textAlign: "right", padding: "6px 8px", fontSize: "9px", fontWeight: 600, textTransform: "uppercase", color: "#4a5568", borderBottom: "2px solid #e2e8f0" }}>Total</th>
              </tr>
            </thead>
            <tbody>
              {productLines.map((line, i) => (
                <tr key={line.id} style={{ borderBottom: "1px solid #edf2f7" }}>
                  <td style={{ padding: "5px 8px", fontSize: "10px" }}>{i + 1}</td>
                  <td style={{ padding: "5px 8px", fontSize: "10px" }}>
                    {line.item_name}
                    {line.description_override && <div style={{ fontSize: "9px", color: "#718096" }}>{line.description_override}</div>}
                  </td>
                  <td style={{ textAlign: "right", padding: "5px 8px", fontSize: "10px", fontFamily: "monospace" }}>{line.qty}</td>
                  <td style={{ textAlign: "right", padding: "5px 8px", fontSize: "10px", fontFamily: "monospace" }}>{line.unit_sell_price_bbd.toFixed(2)}</td>
                  <td style={{ textAlign: "right", padding: "5px 8px", fontSize: "10px", fontFamily: "monospace" }}>{(line.qty * line.unit_sell_price_bbd).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {/* Rx */}
        {lensLines.length > 0 && lensLines.some(l => rxMap[l.id]) && (
          <div style={{ marginBottom: "20px" }}>
            <div style={{ fontSize: "10px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.5px", color: "#718096", marginBottom: "8px" }}>Prescription Details</div>
            {lensLines.map((line) => {
              const rx = rxMap[line.id];
              if (!rx) return null;
              return (
                <div key={line.id} style={{ marginBottom: "12px" }}>
                  <div style={{ fontSize: "10px", fontWeight: 600, color: "#2b6cb0", marginBottom: "4px" }}>{line.item_name}</div>
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ background: "#f7fafc" }}>
                        {["Rx","SPH","CYL","AXIS","FPD","NPD","ADD","Seg","OC","BC"].map(h => (
                          <th key={h} style={{ border: "1px solid #e2e8f0", padding: "2px 4px", fontSize: "9px", textTransform: "uppercase", color: "#4a5568", textAlign: "center" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {(["od","os"] as const).map(eye => (
                        <tr key={eye}>
                          <td style={{ border: "1px solid #e2e8f0", padding: "2px 4px", fontSize: "9px", fontWeight: 600, background: "#f7fafc", textAlign: "left" }}>{eye.toUpperCase()}</td>
                          <td style={{ border: "1px solid #e2e8f0", padding: "2px 4px", fontSize: "9px", textAlign: "center" }}>{fmt((rx as any)[`${eye}_sph`])}</td>
                          <td style={{ border: "1px solid #e2e8f0", padding: "2px 4px", fontSize: "9px", textAlign: "center" }}>{fmt((rx as any)[`${eye}_cyl`])}</td>
                          <td style={{ border: "1px solid #e2e8f0", padding: "2px 4px", fontSize: "9px", textAlign: "center" }}>{fmt((rx as any)[`${eye}_axis`])}</td>
                          <td style={{ border: "1px solid #e2e8f0", padding: "2px 4px", fontSize: "9px", textAlign: "center" }}>{fmt((rx as any)[`${eye}_fpd`])}</td>
                          <td style={{ border: "1px solid #e2e8f0", padding: "2px 4px", fontSize: "9px", textAlign: "center" }}>{fmt((rx as any)[`${eye}_npd`])}</td>
                          <td style={{ border: "1px solid #e2e8f0", padding: "2px 4px", fontSize: "9px", textAlign: "center" }}>{fmt((rx as any)[`${eye}_add`])}</td>
                          <td style={{ border: "1px solid #e2e8f0", padding: "2px 4px", fontSize: "9px", textAlign: "center" }}>{eye === "od" ? (rx.seg_height || "") : (rx.fitting_height || "")}</td>
                          <td style={{ border: "1px solid #e2e8f0", padding: "2px 4px", fontSize: "9px", textAlign: "center" }}>{fmt((rx as any)[`${eye}_oc`])}</td>
                          <td style={{ border: "1px solid #e2e8f0", padding: "2px 4px", fontSize: "9px", textAlign: "center" }}>{fmt((rx as any)[`${eye}_bc`])}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  {rx.pd && <div style={{ fontSize: "9px", marginTop: "3px", color: "#4a5568" }}>PD: {rx.pd}</div>}
                </div>
              );
            })}
          </div>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "12px" }}>
          <div style={{ width: "240px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0", fontSize: "10px" }}>
              <span style={{ color: "#718096" }}>Subtotal ({quote.currency || "BBD"})</span>
              <span style={{ fontFamily: "monospace" }}>{totals.subtotalSell.toFixed(2)}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0 3px", fontSize: "13px", fontWeight: 700, borderTop: "2px solid #2b6cb0", marginTop: "4px" }}>
              <span>Grand Total ({quote.currency || "BBD"})</span>
              <span style={{ fontFamily: "monospace" }}>{totals.grandTotal.toFixed(2)}</span>
            </div>
          </div>
        </div>
        ...
        {/* Notes */}
        {quote.notes_customer && (
          <div style={{ marginTop: "20px", padding: "12px", background: "#f7fafc", borderRadius: "6px", border: "1px solid #e2e8f0" }}>
            <div style={{ fontSize: "10px", fontWeight: 600, textTransform: "uppercase", color: "#718096", marginBottom: "6px" }}>Notes</div>
            <div style={{ fontSize: "10px", color: "#4a5568", whiteSpace: "pre-wrap" }}>{quote.notes_customer}</div>
          </div>
        )}

        <div style={{ marginTop: "28px", borderTop: "1px solid #e2e8f0", paddingTop: "12px", textAlign: "center", fontSize: "9px", color: "#a0aec0" }}>
