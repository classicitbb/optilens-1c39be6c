          ...
          <tbody className="divide-y">
            {isLoading || rxDraftsLoading ? (
              ...
              <>
                ...
                {rxDrafts.map((draft) => { const payload = (draft.input_payload ?? {}) as any; const orderNumber = typeof payload.orderNo === "string" ? payload.orderNo.trim() : ""; const listName = orderNumber && !draft.name.includes(orderNumber) ? `${draft.name} · Order ${orderNumber}` : draft.name; return <Fragment key={draft.id}><tr>
                  <td className="max-w-sm px-4 py-3"><p className="truncate font-medium">{listName}</p><p className="mt-0.5 text-xs text-muted-foreground">Not submitted to the lab</p></td>
