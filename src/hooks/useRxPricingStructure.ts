...
export const useRxPricingStructure = (versionId: number | null) => {
  ...
  const createGrouping = useMutation({
    mutationFn: async ({ name }: { name: string }) => {
      ...
      const { data: groupingRaw, error: groupingError } = await (supabase.from("rx_price_groupings") as any)
        .insert({
          key: uniqueKey,
      ...
      if (sharedCategoryDefaults.length > 0) {
        const { data: newCategoryRows, error: categoryInsertError } = await (supabase.from("rx_price_categories") as any)
      ...
      if (insertedCategories.length > 0 && versionsList.length > 0) {
        const categoryById = new Map((query.data?.categories ?? []).map((category) => [category.id, category]));
        const { data: allExistingCategoryVersions, error: allCategoryVersionsError } = await (supabase.from("rx_price_category_versions") as any)
        ...
        const { error: categoryVersionInsertError } = await (supabase.from("rx_price_category_versions") as any)
          .insert(categoryVersionRows);
        if (categoryVersionInsertError) throw categoryVersionInsertError;

        const allocationRows = versionsList.flatMap((version) =>
  ...
  const createCategory = useMutation({
    mutationFn: async ({ name }: { name: string }) => {
      ...
      const { data: insertedCategories, error: categoryInsertError } = await (supabase.from("rx_price_categories") as any)
  ...
  const renameGrouping = useMutation({
    mutationFn: async ({ groupingId, name }: { groupingId: number; name: string }) => {
      ...
      const { error } = await (supabase.from("rx_price_groupings") as any)
        .update({ default_name: trimmed, updated_at: now })
        .eq("id", groupingId);
      if (error) throw error;

      const { error: versionError } = await (supabase.from("rx_price_grouping_versions") as any)
